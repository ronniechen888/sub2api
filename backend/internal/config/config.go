...
type GatewayConfig struct {
	...
	// OpenAIImagesAllowOAuthFallback: OpenAI 图片 native 请求找不到 API Key 账号时，是否允许回退到 OAuth。
	// 默认开启以支持仅配置 OAuth 账号的部署；如需强制图片只走官方 API Key，可关闭。
	OpenAIImagesAllowOAuthFallback bool `mapstructure:"openai_images_allow_oauth_fallback"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_images_allow_oauth_fallback", true)
